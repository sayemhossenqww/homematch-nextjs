...
import { readRenovationGuide } from "@/lib/guideContent";
...
export async function generateStaticParams() {
  return guides
    .filter((g) => g.section === "renovation-guides")
    .map((g) => ({ slug: g.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = guides.find(
    (g) => g.slug === slug && g.section === "renovation-guides"
  );
  if (!guide) return {};
  return {
    title: guide.title,
    ...
    alternates: {
      canonical: `https://www.homematch.sg/renovation-guides/${slug}`,
    },
  };
}

export default async function RenovationGuideArticle({ params }: Props) {
  const { slug } = await params;
  const guide = guides.find(
    (g) => g.slug === slug && g.section === "renovation-guides"
  );
  ...
  // Recommended reads from same categories
  ...
  // Extract full blog-content-div HTML from the base HTML file
  const contentHtml = await readRenovationGuide(slug);
      ...
      <section className="blog-container">
        <main className="blog-contentwrapper">
          ...
          {contentHtml ? (
            ...
            // Fallback if base HTML file not found
            <aside className="blog-content-div">
              <main className="blog-rich custom-rich-text w-richtext">
                ...
                <p>
                  ...
                  <a
                    href={`https://www.homematch.sg/renovation-guides/${slug}`}
                    target="_blank"
