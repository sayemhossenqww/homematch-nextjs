...
export const metadata: Metadata = { title: "FindMyID Leads" };

const STATUS_OPTS = ["all", "new", "contacted", "matched", "closed", "spam"];
...
export default async function LeadsPage({
  ...
  let query = supabase
    .from("leads")
    .select("id, name, email, whatsapp, property_type, hdb_type, condo_type, budget, status, created_at, referral_source", { count: "exact" })
  ...
  const statusCounts = await Promise.all(
    STATUS_OPTS.filter(s => s !== "all").map(async (s) => {
      const { count: c } = await supabase.from("leads").select("*", { count: "exact", head: true }).eq("status", s);
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <p className="text-[11px] font-black uppercase tracking-widest text-[#c8881f] mb-1">Leads</p>
          <h1 className="text-2xl font-extrabold text-white">FindMyID Requests</h1>
          <p className="text-sm text-white/40 mt-1">{total} {statusFilter === "all" ? "total" : statusFilter} leads</p>
        </div>
      </div>
        ...
        {STATUS_OPTS.map((s) => {
          ...
          return (
            <Link key={s} href={`/admin/leads?status=${s}`}
              ...
              style={{
                background: active ? "rgba(200,136,31,0.14)" : "rgba(255,255,255,0.05)",
                border: `1px solid ${active ? "rgba(200,136,31,0.3)" : "rgba(255,255,255,0.08)"}`,
                color: active ? "#c8881f" : "rgba(255,255,255,0.45)",
              }}>
              {s}
            ...
            {leads.map((lead) => (
              <Link key={lead.id} href={`/admin/leads/${lead.id}`}
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-white truncate group-hover:text-[#c8881f] transition-colors">{lead.name}</p>
                  <p className="text-[11px] text-white/35 truncate mt-0.5">{lead.email}</p>
                  <p className="text-[11px] text-white/25 truncate">{lead.whatsapp}</p>
                </div>

                <div className="min-w-0">
                  <p className="text-xs text-white/65 capitalize">{lead.property_type ?? "—"}</p>
                  <p className="text-[11px] text-white/35 mt-0.5">
                    {lead.hdb_type ?? lead.condo_type ?? ""}
                  </p>
                </div>

                <div>
                  <p className="text-xs text-white/65">{lead.budget ?? "—"}</p>
                </div>

                <div>
                  <p className="text-[11px] text-white/40 truncate">{lead.referral_source ?? "—"}</p>
                </div>
                ...
                <div className="flex items-center gap-1 justify-between">
                  ...
                  <ChevronRight className="w-3.5 h-3.5 text-white/20 group-hover:text-[#c8881f] transition-colors" />
                </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link href={`/admin/leads?status=${statusFilter}&page=${page - 1}`}
