...
const STATUS_OPTS = ["unread", "read", "replied"];
...
export default function ContactActions({ message }: { message: ContactMessage }) {
  ...
  async function handleSave() {
    ...
    await supabase.from("contact_messages").update({ status, admin_notes: notes }).eq("id", message.id);
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div className="rounded-2xl p-5 space-y-4" style={{ background: "rgba(167,139,250,0.05)", border: "1px solid rgba(167,139,250,0.18)" }}>
      <p className="text-[11px] font-black uppercase tracking-widest text-[#a78bfa]">Admin Actions</p>
      ...
      <button onClick={handleSave} disabled={saving}
        ...
        style={{ background: saving ? "rgba(255,255,255,0.08)" : "rgba(167,139,250,0.2)", border: "1px solid rgba(167,139,250,0.35)" }}>
